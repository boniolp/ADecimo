...
with tab_acc:
    ...
    st.dataframe(df[[method + '_score' for method in methods if method not in old_method] + [old_method]])
    
with tab_time:
    st.markdown('# Execution Time Evaluation')
    st.dataframe(df[[method + '_inf' for method in methods if method not in old_method] + [old_method]])
